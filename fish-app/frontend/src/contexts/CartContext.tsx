...
import { Fish, CartItem } from '@/lib/api';
  ...
  switch (action.type) {
    case 'ADD_ITEM': {
      ...
      if (existingItem) {
        ...
        const newItems = [...state.items, { fish, quantity }];
        return {
          ...state,
          items: newItems,
          total: newItems.reduce((sum, item) => sum + (item.fish.price * item.quantity), 0),
    ...
    case 'UPDATE_QUANTITY': {
      ...
      if (quantity <= 0) {
        return cartReducer(state, { type: 'REMOVE_ITEM', payload: { fishId } });
      }
      
      const updatedItems = state.items.map(item =>
        item.fish._id === fishId ? { ...item, quantity } : item
      );
      return {
        ...state,
        items: updatedItems,
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('fish-cart');
    if (savedCart) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('fish-cart', JSON.stringify(state.items));
  }, [state.items]);
